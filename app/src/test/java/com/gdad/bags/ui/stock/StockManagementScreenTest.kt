...
@Test fun ownerSeesCostLotsHistoryAndAdjustment(){render(UserRole.OWNER);compose.onNodeWithText("Adjust stock").assertIsDisplayed();compose.onNodeWithText("Stock value NPR 350.00").assertIsDisplayed();compose.onNodeWithText("FIFO lots").assertIsDisplayed()}
@Test fun salesmanSeesOnHandButNoCostOrMutation(){render(UserRole.SALESMAN);compose.onNodeWithText("SKU B-1 • On hand 7").assertIsDisplayed();compose.onAllNodesWithText("Adjust stock").assertCountEquals(0);compose.onAllNodesWithText("Stock value NPR 350.00").assertCountEquals(0)}
