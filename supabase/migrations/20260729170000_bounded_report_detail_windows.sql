...
declare actor_role public.shop_role; sales_total bigint; return_total bigint; sale_cost bigint;
  ...
  low_stock_count integer; low_stock jsonb; vendor_dues jsonb; vendor_due_total bigint;
  account_balances jsonb; result_payload jsonb;
begin
  ...
  select count(*)::integer into low_stock_count from public.products product
  where product.shop_id=p_shop_id and product.active
  ...
  from (select candidate.id,candidate.sku_code,candidate.name,candidate.current_stock,
      candidate.low_stock_threshold
    from public.products candidate where candidate.shop_id=p_shop_id and candidate.active
      and candidate.current_stock<=candidate.low_stock_threshold
    order by candidate.current_stock,candidate.name,candidate.id limit 501) product;
  result_payload:=jsonb_build_object('shop_id',p_shop_id,'role',actor_role,'date_from',p_date_from,
    ...
    'low_stock_count',low_stock_count,'low_stock_products',low_stock);
  ...
  if actor_role='owner' then
    ...
    from (select vendor.id vendor_id,vendor.display_name vendor_name,
      ...
      group by vendor.id,vendor.display_name having coalesce(sum(private.vendor_bill_due(bill.id)),0)>0
      order by vendor.display_name,vendor.id limit 501) due;
    ...
    from (select candidate.id,candidate.display_name,candidate.account_type
      from public.financial_accounts candidate where candidate.shop_id=p_shop_id
        and candidate.account_type in ('cash','bank') and candidate.active
      order by candidate.account_type,candidate.display_name,candidate.id limit 501) account;
...
comment on function public.get_business_report(uuid,date,date) is
  'Tenant-safe Nepal-date report with exact totals and deterministic 501-row sentinel detail windows.';
