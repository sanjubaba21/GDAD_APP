...
select like(pg_temp.report_explain($$select sum(grand_total_paisa) from public.sales where shop_id='a5800000-0000-4000-8000-000000000001' and business_date between '2026-07-20' and '2026-07-21'$$)::text,'%sales_shop_business_date_idx%','sales period plan uses existing date index');
select like(pg_temp.report_explain($$select sum(total_value_paisa) from public.sale_returns where shop_id='a5800000-0000-4000-8000-000000000001' and status='posted' and business_date between '2026-07-20' and '2026-07-21'$$)::text,'%sale_returns_shop_business_date_idx%','return period plan uses new date index');
select like(pg_temp.report_explain($$select sum(amount_paisa) from public.expenses where shop_id='a5800000-0000-4000-8000-000000000001' and business_date between '2026-07-20' and '2026-07-21'$$)::text,'%expenses_shop_date_category_idx%','expense period plan uses existing date index');
select like(pg_temp.report_explain($$select sum(debit_paisa-credit_paisa) from public.journal_entries where shop_id='a5800000-0000-4000-8000-000000000001' and financial_account_id='a5c00000-0000-4000-8000-000000000001'$$)::text,'%journal_entries_account_timeline_idx%','account balance plan uses existing account index');
