...
select results_eq($$select lot_id,quantity,unit_cost_paisa from public.sale_lot_allocations where sale_line_id=(select id from public.sale_lines where sale_id=(select sale_id from private.sale_operation_requests where idempotency_key='owner-credit')) order by created_at,id$$,$$select * from (values('a3900000-0000-4000-8000-000000000001'::uuid,2,300::bigint),('a3900000-0000-4000-8000-000000000002'::uuid,2,400::bigint)) expected(lot_id,quantity,unit_cost_paisa) order by lot_id$$,'allocation consumes oldest lot then next lot');
