...
class DesktopPurchaseManagementRepository(
    private val remote: PurchaseRemoteDataSource,
    private val products: ProductCatalogRepository,
) : PurchaseManagementRepository {
    private val directory = MutableStateFlow(PurchaseDirectory())

    override fun observe(session: UserSession): Flow<PurchaseDirectory> = directory.asStateFlow()
    ...
    override suspend fun refresh(session: UserSession): PurchaseResult<Unit> {
        ...
        return when (val result = remote.load(session.owner())) {
            ...
            is RemoteResult.Success -> {
                directory.value = PurchaseDirectory(
                    vendors = result.value.vendors.map {
                        Vendor(it.id, it.name, it.phone, it.taxReference, it.notes, it.duePaisa, it.active)
                    },
                    accounts = result.value.accounts.map {
                        PurchaseAccount(it.id, it.name, it.type, it.balancePaisa, it.active)
                    },
                )
    ...
    override suspend fun manageVendor(
        ...
        if (!session.isValidOwner()) return denied()
        if (!requestId.isUuid() || !draft.valid(mutation)) return invalidVendor()
        return when (val result = remote.manageVendor(session.owner(), requestId, mutation, draft)) {
            ...
            is RemoteResult.Success -> when (val refreshed = refresh(session)) {
                ...
                is PurchaseResult.Success -> PurchaseResult.Success(
                    Unit,
                    when (mutation) {
    ...
    override suspend fun postPurchase(
        ...
        if (!session.isValidOwner()) return denied()
        if (!requestId.isUuid() || !draft.valid()) return invalidPurchase()
    ...
    fun snapshot(): PurchaseDirectory = directory.value

    fun clear() {
        directory.value = PurchaseDirectory()
    }

    private fun VendorDraft.valid(mutation: VendorMutation): Boolean = when (mutation) {
    ...
    private fun PurchaseDraft.valid(): Boolean = vendorId.isUuid() &&
        ...
        lines.all {
            it.productId.isUuid() && it.quantity > 0 && it.unitCostPaisa >= 0 &&
                runCatching { it.lineTotalPaisa }.isSuccess
        } && paymentAmountPaisa >= 0 &&
        (paymentAmountPaisa == 0L) == (paymentMethod == null) &&
        runCatching { lines.map { it.lineTotalPaisa } }.getOrNull()
            ?.let(MoneyAmounts::sumPaisa)?.let { paymentAmountPaisa <= it } == true

    private fun RemoteFailure.failure(default: String) = PurchaseResult.Failure(
        this,
        when (kind) {
    ...
    private fun invalidVendor() = PurchaseResult.Failure(null, "Review the vendor fields.")
    private fun invalidPurchase() =
        PurchaseResult.Failure(null, "Review the date, invoice, lines, quantities, costs, and payment.")
